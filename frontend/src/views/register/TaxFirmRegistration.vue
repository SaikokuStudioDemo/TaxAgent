    ...
    if (validUsers.length > 0) {
      ...
      if (!employeesResponse.ok) {
        ...
        if (failedEmps.length > 0) {
          ...
          alert(`税理士法人の登録は完了しましたが、以下のメールアドレスのアカウント作成に失敗しました（既に登録されている可能性があります）：\n${failedEmails}`);
        }
      }
    }
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-24">
    ...
    <div class="mb-10">
      ...
      <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight mb-3">税理士法人 新規登録</h1>
      <div class="w-20 h-1 bg-indigo-600 rounded-full mb-4"></div>
      <p class="text-gray-600 max-w-2xl text-lg">
        Tax-Agentの利用を開始するためのアカウント作成と初期設定を行います。
      </p>
    </div>
    ...
    <form @submit.prevent="onSubmit" class="flex flex-col lg:flex-row gap-8 items-start">
      ...
      <div class="w-full lg:w-2/3">
        <ContractInfoForm
          salesAgentName="山田 太郎（第一営業部）"
          referrerName="株式会社テスト会計事務所"
          :isTaxFirm="true"
