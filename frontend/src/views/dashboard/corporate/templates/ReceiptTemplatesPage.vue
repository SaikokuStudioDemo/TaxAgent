...
import { Receipt, Plus, Edit2, Trash2, Eye } from 'lucide-vue-next';
...
const form = ref({
  ...
  description: '',
  html: '',
  thumbnail: 'bg-green-50 border-green-200',
  is_default: false,
...
const fetchTemplates = async () => {
  ...
  try {
    const data = await api.get<Template[]>('/receipts/templates?template_type=receipt');
...
const handleSave = async () => {
  ...
  try {
    if (editingId.value) {
      await api.put(`/receipts/templates/${editingId.value}`, form.value);
    } else {
      await api.post('/receipts/templates', form.value);
    }
    resetForm();
...
const handleEdit = async (t: Template) => {
  const detail = await api.get<any>(`/receipts/templates/${t.id}`);
  editingId.value = t.id;
  form.value = { name: detail.name, description: detail.description, html: detail.html || '', thumbnail: detail.thumbnail, is_default: detail.is_default, template_type: 'receipt' };
  showForm.value = true;
};
...
const handleDelete = async (t: Template) => {
  ...
  try {
    await api.delete(`/receipts/templates/${t.id}`);
    await fetchTemplates();
  } catch (e: any) {
    error.value = e.message;
  }
...
const resetForm = () => {
  ...
  form.value = { name: '', description: '', html: '', thumbnail: 'bg-green-50 border-green-200', is_default: false, template_type: 'receipt' };
};
  ...
  <div class="p-6 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <Receipt class="text-green-600" :size="22" />
        <div>
          <h1 class="text-xl font-bold text-slate-800">領収書テンプレート</h1>
          <p class="text-sm text-slate-500">領収書の HTML テンプレートを管理します</p>
        </div>
      </div>
    ...
    <div v-else-if="templates.length === 0"
      ...
      <Receipt :size="40" class="mx-auto mb-3 opacity-30" />
    ...
    <Teleport to="body">
      <div v-if="showForm" class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
          ...
          <div class="space-y-4">
            <div><label class="text-sm font-medium block mb-1">テンプレート名</label>
              <input v-model="form.name" type="text" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div><label class="text-sm font-medium block mb-1">説明</label>
              <input v-model="form.description" type="text" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div><label class="text-sm font-medium block mb-1">HTML テンプレート</label>
              <textarea v-model="form.html" rows="8" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          <div class="flex justify-end gap-3 mt-6">
            <button @click="resetForm" class="px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600">キャンセル</button>
    ...
    <Teleport to="body">
      <div v-if="showPreview" class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60" @click.self="showPreview = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-auto p-6">
          <div class="flex justify-between items-center mb-4">
            ...
            <button @click="showPreview = false" class="text-slate-400 text-xl">×</button>
          </div>
          <div v-html="previewHtml" class="border border-slate-200 rounded-lg p-4" />
        </div>
      </div>
    </Teleport>
  </div>
